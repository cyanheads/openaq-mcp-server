...
await createApp({
  ...
  instructions:
    'Measured air quality from physical sensors (OpenAQ v3) — the ground-truth counterpart to modeled air-quality grids. Workflow: openaq_find_locations (find stations near a point / in a bbox / by country) → openaq_get_readings (latest values) or openaq_get_measurements (historical series). An empty find_locations result means NO monitoring coverage, NOT clean air — widen the search area, check openaq_list_countries, or fall back to open-meteo-mcp-server for modeled coverage. Units vary by sensor and are NEVER converted; the same pollutant has multiple parameter ids for different units (use openaq_list_parameters to disambiguate). Large measurement series stage on a DataCanvas (requires CANVAS_PROVIDER_TYPE=duckdb) — call openaq_dataframe_describe on the returned canvasId for the table columns, then openaq_dataframe_query to run SQL.',
  setup(core) {
