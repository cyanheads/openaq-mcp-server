...
In this server the four upstream reasons (`upstream_error`, `rate_limited`, `upstream_timeout`, `invalid_api_key`) are raised by `upstreamFailure` in `src/mcp-server/tools/shared/upstream-errors.js`, below every handler — so each definition declaring them marks them `thrownBy: 'service'`. `canvas_not_found` and `missing_table` on the dataframe tools come from the canvas the same way. A reason the handler itself throws stays unmarked so the rule keeps checking it.
